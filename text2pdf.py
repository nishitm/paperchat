...
class PyText2Pdf(object):
    ...
    def parse_args(self, inputFile):
        ...
        parser.add_option('-s','--size',dest='fontsize',help='Use font at PTSIZE points (default=>6)',metavar='PTSIZE',default=6)
        parser.add_option('-v','--linespace',dest='linespace',help='Use line spacing LINESPACE (deault 7)',metavar='LINESPACE',default=7)
        parser.add_option('-l','--lines',dest='lines',help='Lines per page (default 120, determined automatically if unspecified)',default=120, metavar=None)
        parser.add_option('-c','--chars',dest='chars',help='Maximum characters per line (default 152)',default=152,metavar=None)
        ...
        parser.add_option('-W','--width',dest='width',help='Independent paper width in points',metavar=None,default=662) #612)
    ...
    def convert(self):
        ...
        try:
            ...
            return 'Error: Could not open file to read --->', self._ifile
            #sys.exit(3)
        ...
        try:
            ...
            return 'Error: Could not open file to write --->', self._ofile
            #sys.exit(3)
            ...
            while column <= self._columns:
                ...
                while lineNo < self._lines and not atFF and not atEOF and not pagebreak:
                    ...
                    while charNo < self._cols:
                        ...
                        if ord(ch) >= 32 and ord(ch) <= 127 and ch!='!':
                            if ch == '(' or ch == ')' or ch == '\\':
                                ws("\\")
                            ws(ch)
                        else:
                            if ch == '!':
                              ...
                              if LARGE==1:
                                ...
                                LARGE=1
                                buf = "".join(("/F1 ", str(12), " Tf\n"))
                              ws(buf)
                              if LARGE==1:
                                buf = "".join(("1 0 0 1 ",str(50+3.5*charNo+2)," ", str(self._pageHt - 40-(lineNo-1)*self._vertSpace), " Tm\n"))
                                charNo-=1
                              else:
                                buf = "".join(("1 0 0 1 50 ", str(self._pageHt - 40-(lineNo-1)*self._vertSpace), " Tm\n"))
                                charNo=1;
                              ws(buf)
                              buf = "7 TL\n"
                              ws(buf)
